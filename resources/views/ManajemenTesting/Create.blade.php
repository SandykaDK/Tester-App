...
<body class="bg-gray-100 relative min-h-screen overflow-x-hidden">
    ...
    <div class="main-content flex-1 p-6 ml-64 relative z-10">
        ...
        <div class="bg-white p-6 rounded shadow">
            <h1 class="text-2xl font-bold mb-4">Input Test Case</h1>
            ...
            <form method="POST" action="{{ route('test-cases.store') }}">
                ...
                <div id="wizard-step-1" class="wizard-step">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="modul_key" id="modul_key" class="w-full border rounded p-2" disabled>
                            <option value="">Pilih Modul</option>
                            @foreach ($modules as $module)
                                <option value="{{ $module->id }}" data-app-id="{{ $module->application_id }}">{{ $module->modul_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="menu_key" class="block text-sm font-medium text-gray-700 mb-2">Menu</label>
                        <select name="menu_key" id="menu_key" class="w-full border rounded p-2" disabled>
                            <option value="">Pilih Menu</option>
                            @foreach ($menus as $menu)
                                <option value="{{ $menu->id }}" data-module-id="{{ $menu->modul_id }}">{{ $menu->menu_name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div id="wizard-step-2" class="wizard-step hidden">
                    <div class="mb-4">
                        ...
                        <textarea name="test_scenario" id="test_scenario" class="w-full border rounded p-2"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="test_data" class="block text-sm font-medium text-gray-700 mb-2">Test Data</label>
                        <textarea name="test_data" id="test_data" class="w-full border rounded p-2"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="test_step" class="block text-sm font-medium text-gray-700 mb-2">Test Step</label>
                        <textarea name="test_step" id="test_step" class="w-full border rounded p-2"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="expected_result" class="block text-sm font-medium text-gray-700 mb-2">Expected Result</label>
                        <textarea name="expected_result" id="expected_result" class="w-full border rounded p-2"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="result" class="block text-sm font-medium text-gray-700 mb-2">Actual Result</label>
                        <textarea name="result" id="result" class="w-full border rounded p-2"></textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="pic_dev" id="pic_dev" class="w-full border rounded p-2">
                            ...
                            @foreach ($developers as $developer)
                                <option value="{{ $developer->id }}">{{ $developer->dev_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="test_date" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Testing</label>
                        <input type="date" name="test_date" id="test_date" class="w-full border rounded p-2">
                    </div>
                    <div class="flex justify-between">
