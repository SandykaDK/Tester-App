...
<body class="bg-gray-100 relative min-h-screen overflow-x-hidden">
    ...
    <div class="main-content flex-1 p-6 ml-64 relative z-10">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-bold mb-4">Manajemen Testing</h1>
            <div class="mb-4 flex justify-between items-end">
                ...
                <a href="{{ route('test-cases.create') }}" class="mt-6 px-3 py-1 bg-green-500 text-white rounded hover:bg-green-700 flex items-center">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Modul</th>
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Menu</th>
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">PIC Dev</th>
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Test Date</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($testCases as $index => $testCase)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="py-2 px-4 text-sm font-medium text-gray-900">{{ $testCase->application->app_name ?? '-' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $testCase->module->modul_name ?? '-' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $testCase->menu->menu_name ?? '-' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $testCase->status }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $testCase->developer->dev_name ?? '-' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $testCase->test_date }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('test-cases.edit', $testCase->id) }}" class="inline-block p-2 bg-yellow-400 text-white rounded hover:bg-yellow-700">
                                        ...
                                        <button type="button" class="delete-button inline-block p-2 bg-red-500 text-white rounded hover:bg-red-700" data-action="{{ route('test-cases.destroy', $testCase->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
