...
<head>
    ...
    <title>Edit Screen Format</title>
...
<body class="bg-gray-100 relative min-h-screen overflow-x-hidden">
    ...
    <div class="main-content flex-1 p-6 relative z-10">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-bold mb-4">Edit Screen Format</h1>
            <form action="{{ route('screen-format.update', $screenFormat->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="scr_file" class="block text-sm font-medium text-gray-700">Upload File</label>
                    <input type="file" name="scr_file" id="scr_file" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 @error('scr_file') border-red-500 @enderror">
                    @if($screenFormat->scr_name)
                        <div class="text-xs text-gray-500 mt-1">File saat ini: <span class="font-semibold">{{ $screenFormat->scr_name }}</span></div>
                    @endif
                    @error('scr_file')
                ...
                <div class="mb-4 relative">
                    ...
                    <input type="text" id="scr_menu" value="{{ old('scr_menu', $screenFormat->scr_menu ?? '') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 @error('menu_id') border-red-500 @enderror" autocomplete="off" required>
                    <input type="hidden" name="menu_id" id="menu_id" value="{{ old('menu_id', $screenFormat->menu_id ?? '') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="scr_version" id="scr_version" value="{{ old('scr_version', $screenFormat->scr_version) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 @error('scr_version') border-red-500 @enderror" required>
                    @error('scr_version')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <textarea name="scr_description" id="scr_description" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 @error('scr_description') border-red-500 @enderror">{{ old('scr_description', $screenFormat->scr_description) }}</textarea>
