    ...
    <div class="main-content flex-1 p-6 relative z-10">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="mb-4 flex justify-between items-end">
                <form method="GET" action="{{ route('menus.index') }}" class="flex items-end space-x-2">
                    <div>
                        <label for="menu_name" class="block text-sm font-medium text-gray-700">Nama Menu</label>
                        <input type="text" name="menu_name" id="menu_name" value="{{ request('menu_name') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="module_name" class="block text-sm font-medium text-gray-700">Nama Modul</label>
                        <input type="text" name="module_name" id="module_name" value="{{ request('modul_name') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="menu_status" id="menu_status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option value="">Semua</option>
                            <option value="Not Started" {{ request('menu_status') == 'Not Started' ? 'selected' : '' }}>Not Started</option>
                            <option value="On Progress" {{ request('menu_status') == 'On Progress' ? 'selected' : '' }}>On Progress</option>
                            <option value="Active" {{ request('menu_status') == 'Active' ? 'selected' : '' }}>Active</option>
                        </select>
                    </div>
                    <div>
                ...
                <a href="{{ route('menus.create') }}" class="mt-6 px-3 py-1 bg-green-500 text-white rounded hover:bg-green-700 flex items-center">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="py-2 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Menu</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($menus as $index => $menu)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="py-2 px-4 text-sm font-medium text-gray-900">{{ $menu->menu_name }}</td>
                                <td class="py-2 px-4 text-sm font-medium text-gray-900">{{ $menu->application->app_name }}</td>
                                <td class="py-2 px-4 text-sm font-medium text-gray-900">{{ $menu->module ? $menu->module->modul_name : '-' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $menu->menu_description }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">
                                    @if($menu->menu_status === 'Not Started')
                                        <span class="ml-2 px-2 py-1 text-gray-700 font-bold">{{ $menu->menu_status }}</span>
                                    @elseif($menu->menu_status == 'On Progress')
                                        <span class="ml-2 px-2 py-1 text-orange-700 font-bold">{{ $menu->menu_status }}</span>
                                    @elseif($menu->menu_status === 'Active')
                                        <span class="ml-2 px-2 py-1 text-green-700 font-bold">{{ $menu->menu_status }}</span>
                                    @endif
                                </td>
                                <td class="py-2 px-4 text-sm text-gray-500">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('menus.edit', $menu->id) }}" class="inline-block p-2 bg-yellow-400 text-white rounded hover:bg-yellow-700">
                                        ...
                                        <button type="button" class="delete-button inline-block p-2 bg-red-500 text-white rounded hover:bg-red-700" data-action="{{ route('menus.destroy', $menu->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
