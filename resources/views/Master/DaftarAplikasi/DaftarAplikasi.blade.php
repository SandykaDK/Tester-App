...
<body class="bg-gray-100 relative min-h-screen overflow-x-hidden">
    ...
    <div class="main-content flex-1 p-6 relative z-10">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <h1 class="text-2xl font-bold mb-4">Daftar Aplikasi</h1>
            <x-alert-success />
            <x-alert-failure />
            <div class="mb-4 flex justify-between items-end">
                <form method="GET" action="{{ route('applications.index') }}" class="flex items-end space-x-4">
                    ...
                    <div>
                        ...
                        <select name="app_status" id="app_status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option value="">Semua</option>
                            <option value="Not Started" {{ request('app_status') == 'Not Started' ? 'selected' : '' }}>Not Started</option>
                            <option value="On Progress" {{ request('app_status') == 'On Progress' ? 'selected' : '' }}>On Progress</option>
                            <option value="Active" {{ request('app_status') == 'Active' ? 'selected' : '' }}>Active</option>
                        </select>
                    </div>
                    <div>
                ...
                <a href="{{ route('applications.create') }}" class="mt-6 px-3 py-1 bg-green-500 text-white rounded hover:bg-green-700 flex items-center">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($applications as $index => $application)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="py-2 px-4 text-sm font-medium text-gray-900">{{ $application->app_name }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">{{ $application->app_description }}</td>
                                <td class="py-2 px-4 text-sm text-gray-500">
                                    @if($application->app_status === 'Not Started')
                                        <span class="ml-2 px-2 py-1 text-gray-700 font-bold">{{ $application->app_status }}</span>
                                    @elseif($application->app_status == 'On Progress')
                                        <span class="ml-2 px-2 py-1 text-orange-700 font-bold">{{ $application->app_status }}</span>
                                    @elseif($application->app_status === 'Active')
                                        <span class="ml-2 px-2 py-1 text-green-700 font-bold">{{ $application->app_status }}</span>
                                    @endif
                                </td>
                                <td class="py-2 px-4 text-sm text-gray-500">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('applications.edit', $application->id) }}" class="inline-block p-2 bg-yellow-400 text-white rounded hover:bg-yellow-700">
                                        ...
                                        <button type="button" class="delete-button inline-block p-2 bg-red-500 text-white rounded hover:bg-red-700" data-action="{{ route('applications.destroy', $application->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
